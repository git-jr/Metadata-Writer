...
try:
    from tflite_support._cc import _pywrap_metadata_version
except ImportError:
    _pywrap_metadata_version = None
try:
    from tflite_support._flatbuffers_lib import _pywrap_flatbuffers
except ImportError:
    _pywrap_flatbuffers = None
...
class MetadataPopulator(object):
  ...
  def load_metadata_buffer(self, metadata_buf):
    ...
    min_version = (_pywrap_metadata_version.GetMinimumMetadataParserVersion(bytes(metadata_buf))
                   if _pywrap_metadata_version is not None else "1.0.0")
